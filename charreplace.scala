...
// function for calculating charreplace typo
def charrep(x:String,y:String):Int ={

var urllen= x.length
var c = 0
var f = 0
var foundIt = false 
while (c<= urllen-1 && !foundIt){

if (y(c).toInt != x(c).toInt)
{  
  f = f+1
}
if (f==2) foundIt = true
...
val tyo = new PrintWriter("/usr/local/spark/tymain.txt")
...
var t4 = typotype.exists(s=>s=="charrep")
...
// charreplace typo starts

if (t4==true)
{
if (typo(j).length==url(i).length)
{
var chrep = charrep(url(i),typo(j))
if (chrep==1 && url(i)!=typo(j)){
var f = typo(j)::Nil
ty4 = ty4:::f
}
}
}
// charreplace typo ends
// typo3 starts 
   if (t3==true){
...
if (ty4.length>10)
{
println("The length of ty4 is "+ty4.length)
var b = 0
while(b<ty4.length)
{
tyo.println(ty4(b))
...
// experimental part ends
if (t4==true)pw.println("The List of Charreplace for "+url(i)+ " is "+ty4.drop(1))
pw.println("")   
println(ty4.drop(1).length)
